...
if (!empty($eventPackages[$packageKey]['archived'])) {
    ...
    echo json_encode(['ok' => false, 'message' => 'Event package is already archived']);
    exit;
}

$eventPackages[$packageKey]['archived'] = true;
$eventPackages[$packageKey]['archivedAt'] = date('Y-m-d H:i:s');

if (!save_event_packages_repository($eventPackages)) {
    http_response_code(500);
    echo json_encode(['ok' => false, 'message' => 'Unable to archive event package']);
    exit;
}
...
echo json_encode([
    ...
    'archivedEntry' => [
        ...
        'archivedAt' => (string) ($eventPackages[$packageKey]['archivedAt'] ?? '')
    ]
]);
